...
const Rotote_authMessage = document.getElementById('auth-message');
...
L_btn.addEventListener('click', function (event) {
    ...
    signInWithEmailAndPassword(auth, L_email, L_password)
        .then((userCredential) => {
            ...
            Rotote_authMessage.classList.add('hidden');
            window.isUserLoggedIn = true;
            document.getElementById('auth-status-msg').textContent = `当前用户: ${L_email} `;
            setTimeout(() => {
                window.updateAuthUI();
                window.closeAuthModal();
            }, 1000);
